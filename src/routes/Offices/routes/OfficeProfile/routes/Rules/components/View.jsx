...
class View extends Component {
  static propTypes = {
    ...
    createOfficeRule: PropTypes.func.isRequired,
  ...
  handleAddRule = async (variables) => {
    ...
    const { data: { rules: { createRule: { data, error } } } } = await createOfficeRule(
      {
        variables: {
          createdBy,
          actions: [action],
    ...
    if (error) {
      notify({
        ...
        message: I18n.t('OFFICES.TABS.RULES.RULE_NOT_CREATED'),
      });
      ...
      notify({
        ...
        message: I18n.t('OFFICES.TABS.RULES.RULE_CREATED', { id: data.uuid }),
      });
    }
  };
  ...
  handleDeleteRule = uuid => async () => {
    ...
    if (error) {
      ...
      notify({
        level: 'error',
        title: I18n.t('COMMON.FAIL'),
        message: I18n.t('OFFICES.TABS.RULES.RULE_NOT_DELETED'),
      });
    } else {
      await refetch();
      deleteModal.hide();
      notify({
        level: 'success',
        title: I18n.t('COMMON.SUCCESS'),
        message: I18n.t('OFFICES.TABS.RULES.RULE_DELETED', { id: data.uuid }),
      });
    }
  }
  ...
  handleDeleteRuleClick = (uuid) => {
    ...
    deleteModal.show({
      ...
      modalTitle: I18n.t('OFFICES.TABS.RULES.DELETE_MODAL.HEADER'),
      actionText: I18n.t('OFFICES.TABS.RULES.DELETE_MODAL.ACTION_TEXT', { name }),
      submitButtonLabel: I18n.t('OFFICES.TABS.RULES.DELETE_MODAL.DELETE'),
    });
  };
    ...
    return (
      <Fragment>
        <TabHeader title={I18n.t('PLAYER_PROFILE.PROFILE.TITLE')}>
          ...
          >
            + {I18n.t('OFFICES.TABS.RULES.ADD_RULE')}
          </button>
        </TabHeader>
        <RulesFilters
        ...
        <div className="card-body">
          ...
          >
            <GridViewColumn
              name="rule"
              header={I18n.t('OFFICES.TABS.RULES.GRID_HEADER.RULE')}
              render={this.renderRule}
            />
            <GridViewColumn
              name="countries"
              header={I18n.t('OFFICES.TABS.RULES.GRID_HEADER.COUNTRY')}
              render={this.renderRuleInfo(infoConfig.countries)}
            />
            <GridViewColumn
              name="languages"
              header={I18n.t('OFFICES.TABS.RULES.GRID_HEADER.LANGUAGE')}
              render={this.renderRuleInfo(infoConfig.languages)}
            />
            <GridViewColumn
              name="priority"
              header={I18n.t('OFFICES.TABS.RULES.GRID_HEADER.PRIORITY')}
              render={this.renderPriority}
            />
            <GridViewColumn
              name="delete"
              header={I18n.t('OFFICES.TABS.RULES.GRID_HEADER.ACTION')}
