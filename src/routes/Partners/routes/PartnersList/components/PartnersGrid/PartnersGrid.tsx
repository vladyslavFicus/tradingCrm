...
const PartnersGrid = (props: Props) => {
  ...
  // ===== Renders ===== //
  const renderPartner = ({ uuid, fullName }: Partner) => (
    <>
  ...
  const renderExternalAffiliateId = ({ externalAffiliateId }: Partner) => (
    <If condition={!!externalAffiliateId}>
      <div>{externalAffiliateId}</div>
    </If>
  );

  const renderCountry = ({ country }: Partner) => (
    <Choose>
  ...
  const renderRegistered = ({ createdAt }: Partner) => (
    <>
  ...
  const renderStatus = ({ status, statusChangeDate }: Partner) => (
    <>
      <div
        className={
          classNames(
