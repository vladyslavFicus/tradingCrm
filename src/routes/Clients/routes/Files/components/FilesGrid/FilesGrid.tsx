...
const FilesGrid = (props: Props) => {
  ...
  // ===== Renders ===== //
  const renderFullName = ({ clientUuid, client }: File) => (
    <>
      <Choose>
  ...
  const renderFileName = (data: File) => {
  ...
  const renderExpirationTime = ({ expirationDate }: File) => {
    if (!expirationDate) {
  ...
  const renderDate = ({ uploadDate }: File) => (
    <>
  ...
  const renderCategory = ({ verificationType }: File) => (
  ...
  const renderDocumentType = ({ documentType }: File) => (
