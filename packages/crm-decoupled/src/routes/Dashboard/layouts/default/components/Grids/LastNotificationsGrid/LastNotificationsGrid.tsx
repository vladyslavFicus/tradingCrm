...
} from 'routes/Dashboard/graphql/__generated__/LastNotificationsQuery';
...
const LastNotificationsGrid = () => {
  const {
    loading,
    content,
  } = useGrid<LastNotification>(useLastNotificationsQuery, Grids.lastNotifications);

  const renderNotificationType = useCallback(({ type, uuid }: LastNotification) => (
    <If condition={!!type}>
  ...
  const renderNotificationTypeDetails = useCallback(({ type, subtype, details }: LastNotification) => (
    <>
      <div className="LastNotificationsGrid__text-highlight">
  ...
  const renderClient = useCallback(({ client }: LastNotification) => (
    <GridPlayerInfo profile={client} mainInfoClassName="LastNotificationsGrid__text-highlight" />
  ), []);

  const renderNotificationDate = useCallback(({ createdAt }: LastNotification) => {
    if (!createdAt) {
      return null;
    }
  ...
  const renderPriority = useCallback(({ priority }: LastNotification) => (
  ...
  const rowsClassNames = useCallback(({ priority, read }: LastNotification) => classNames(
