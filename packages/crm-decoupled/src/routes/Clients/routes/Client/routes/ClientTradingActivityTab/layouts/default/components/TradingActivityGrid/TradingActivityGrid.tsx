...
import useTradingActivityGrid
  from 'routes/Clients/routes/Client/routes/ClientTradingActivityTab/hooks/useTradingActivityGrid';
...
const TradingActivityGrid = (props: Props) => {
  ...
  const {
    ...
    columnsOrder,
  } = useTradingActivityGrid({ onRefetch });

  // ===== Renders ===== //
  const renderTrade = useCallback((tradingActivity: TradingActivity) => {
  ...
  const renderType = useCallback(({ operationType }: TradingActivity) => (
    <div
  ...
  const renderAccount = useCallback(({ login, symbol, platformType }: TradingActivity) => (
    <>
  ...
  const renderOriginalAgent = useCallback(({ originalAgent }: TradingActivity) => {
    if (!originalAgent) {
  ...
  const renderOpenPrice = useCallback(({ openPrice, stopLoss, takeProfit }: TradingActivity) => (
    <>
  ...
  const rendeTime = useCallback((time?: number | null) => {
    if (!time) {
      return <span>&mdash;</span>;
    }
  ...
  const renderStatus = useCallback(({ tradeStatus }: TradingActivity) => (
    <div
      className={classNames(
